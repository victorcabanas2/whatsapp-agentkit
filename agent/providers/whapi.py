...
class ProveedorWhapi(ProveedorWhatsApp):
    ...
    async def obtener_historial_whapi(self, chat_id: str) -> list[dict]:
        """
        Obtiene el historial completo de mensajes de una conversación en Whapi.
        Incluye TODOS los mensajes previos (no solo últimos 100).

        Args:
            chat_id: Número de teléfono del cliente

        Returns:
            Lista de mensajes: [{"role": "user|assistant", "content": "..."}]
        """
        if not self.token:
            logger.warning("WHAPI_TOKEN no configurado — no se puede obtener historial")
            return []

        chat_id_limpio = str(chat_id).replace("+", "").replace(" ", "")
        headers = {
        ...
        try:
            async with httpx.AsyncClient() as client:
                # Whapi endpoint para obtener historial
                response = await client.get(
                    f"https://gate.whapi.cloud/chats/{chat_id_limpio}/messages",
                    headers=headers,
                    timeout=15.0,
                )

                if response.status_code != 200:
                    logger.warning(f"⚠️ No se pudo obtener historial de Whapi ({response.status_code})")
                    return []
                ...
                for msg in data.get("messages", []):
                    ...
                    role = "assistant" if es_propio else "user"
                ...
                logger.info(f"✓ Historial de Whapi obtenido: {len(mensajes)} mensajes para {chat_id_limpio}")
                return mensajes

        except Exception as e:
            logger.error(f"✗ Error al obtener historial de Whapi: {e}")
            return []
